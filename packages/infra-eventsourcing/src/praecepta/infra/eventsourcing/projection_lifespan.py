...
entry points, creates ProjectionRunners, and manages their lifecycle
(start on app startup, stop on app shutdown).

Priority 200 ensures projections start AFTER the event store (priority 100)
is initialized, since projections depend on the event store infrastructure.
"""
...
async def projection_runner_lifespan(app: Any) -> AsyncIterator[None]:
    ...
    Creates one ProjectionRunner per upstream application, each with
    all discovered projections. Projections filter events via their
    topics attribute, so unmatched projections are effectively no-ops.
    ...
    try:
        for app_cls in applications:
            ...
            logger.info(
                "Started projection runner for %s with %d projection(s)",
                app_cls.__name__,
                len(projections),
            )
    except Exception:
